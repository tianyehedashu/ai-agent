...
| A5 | `speech` / `aspeech` | ✅ | 直连 `litellm.aspeech`（**不经 Router**） | HTTP：`POST .../v1/audio/speech`；结算走 `schedule_settle_usage` |
...
| A17 | `supports_reasoning` | ◐ | `litellm_capability_hint.py` → `LitellmCapabilityHintAdapter` | **仅目录写侧 hint**，不驱动运行时路由 |
| **B. Router** |
...
| **E. 定价与成本** |
| E1 | `litellm.model_cost` 内置价目 | ✅ | 启动 `sync_to_litellm_registry()`（`pricing_service.py`） | |
| E2 | `litellm.register_model()` | ✅ | 合并 DB `upstream_model_pricing` + 虚拟模型名 | deployment 级单价额外注入 Router |
...
| `POST .../openai/v1/audio/speech` | `audio_speech` | `aspeech` | ❌ |
| `POST .../openai/v1/rerank` | `rerank` | `arerank` | ❌ |
| `POST .../openai/v1/moderations` | `moderation` | `amoderation` | ❌ |
| `POST .../openai/v1/videos` | `video_generation` | `avideo_generation` | ❌ |
...
| `gateway_catalog_sync_on_startup` | `False` | 启动时 sync 价目 + `reload_router` |
...
| 现象 | 原因 | 项目处理 |
|------|------|----------|
| DashScope embedding 报错 | LiteLLM provider 映射不完整 | OpenAI 兼容 `/embeddings` 直连（`dashscope_embedding_client`） |
| `aspeech` / `arerank` 等不经 Router | 历史实现；无 deployment 级 failover | 仅依赖全局 callbacks；**无** Router fallback |
...
| LiteLLM 版本升级 | `model_cost` / provider 行为变化 | 跑 `test_litellm_upstream_price_sync`、集成测、`scripts/test_litellm_models.py` |
...
| [GATEWAY_COMPATIBILITY_CHECK.md](./GATEWAY_COMPATIBILITY_CHECK.md) | Agent 与 Gateway 类型兼容 |

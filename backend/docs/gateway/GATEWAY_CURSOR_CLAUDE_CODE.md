...
| **团队绑定** | `sk-gw-*` 创建时写入 `gateway_virtual_keys.tenant_id`，代理按该团队解析模型；**勿**与个人团队 Key 混用，也**勿**传 `X-Team-Id`（与平台 `sk-*` 不同） |

鉴权实现见 `domains/gateway/presentation/deps.py`（`bearer_vkey_or_apikey_auth`）。管理台在 **AI Gateway → 虚拟 Key** 查看 Key 所属团队；404 响应会附带「当前调用团队」便于对照。

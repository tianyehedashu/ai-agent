...
| 字段 | 处理层 | 粒度 | 决策依据 |
|------|--------|------|----------|
| `thinking` / `enable_thinking` | `domain/policies/invocation_policy.py::apply_invocation_kwargs`（经 `UpstreamAdapter`） | **模型**粒度 | `GatewayModel.tags["thinking_param"]`：`anthropic_extended` 保留；`dashscope_enable_thinking` 翻译；`builtin_reasoning` / `none` 剥离 |

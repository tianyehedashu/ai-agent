...
│                       执行器层 (Executor)                                │
│  ┌──────────────────────┐  ┌──────────────────┐  ┌──────────────────┐   │
│  │ PersistentDocker     │  │ DockerExecutor   │  │ LocalExecutor    │   │
│  │ Executor             │  │ (无状态)          │  │ (仅开发)         │   │
│  │ (沙箱级容器)          │  │                   │  │                  │   │
│  └────────┬─────────┘  └────────┬─────────┘  └────────┬─────────┘       │
│           │                     │                     │                  │
...
| 执行器 | 说明 | 适用场景 |
|--------|------|---------|
| `PersistentDockerExecutor` | 沙箱级 Docker 容器，状态保持 | **生产环境推荐** |
| `DockerExecutor` | 无状态 Docker 容器，每次新建 | 一次性任务 |
| `LocalExecutor` | 本地直接执行，无隔离 | 仅开发调试 |

#### PersistentDockerExecutor 特点
...
async with PersistentDockerExecutor(...) as executor:
...
```python
from domains.agent.infrastructure.sandbox import SandboxManager, CleanupReason

# 获取单例实例（或通过 request.app.state.sandbox_manager）
...
# 执行命令
cmd_result = await sandbox.executor.execute_shell("date")

# 状态管理
await manager.mark_sandbox_complete(sandbox.sandbox_id)
await manager.mark_sandbox_disconnected(sandbox.sandbox_id)
await manager.mark_sandbox_reconnected(sandbox.sandbox_id)

# 主动结束
await manager.end_sandbox(sandbox.sandbox_id, CleanupReason.USER_REQUEST)
...
```python
# 通过 request.app.state.sandbox_manager 获取沙箱管理器，或使用 get_sandbox_service(request) 获取适配器（会话联动）
...
async def chat(
    ...
    # 获取或创建沙箱（session_id 为业务会话 ID）
    result = await sandbox_manager.get_or_create(
    ...
    try:
        # 执行 Agent 任务
        result = await agent.run(message, executor=sandbox.executor)

        # 任务完成，标记状态
        await sandbox_manager.mark_sandbox_complete(sandbox.sandbox_id)

        return result
    except Exception as e:
        await sandbox_manager.end_sandbox(sandbox.sandbox_id, CleanupReason.ERROR)
        raise
```
...
async def websocket_endpoint(request: Request, websocket: WebSocket, session_id: str):
    await websocket.accept()
    sandbox_manager = request.app.state.sandbox_manager

    result = await sandbox_manager.get_or_create(session_id=session_id)
    sandbox = result.sandbox

    try:
        while True:
            data = await websocket.receive_text()
            await sandbox_manager.update_sandbox_activity(sandbox.sandbox_id)
            # 处理消息...
    except WebSocketDisconnect:
        await sandbox_manager.mark_sandbox_disconnected(sandbox.sandbox_id)
```
...
async def end_session(request: Request, session_id: str):
    sandbox_manager = request.app.state.sandbox_manager
    sandbox_id = sandbox_manager.get_sandbox_id_by_session(session_id)
    if sandbox_id:
        await sandbox_manager.end_sandbox(sandbox_id, CleanupReason.USER_REQUEST)
    return {"status": "ended"}
```
...
```python
@router.get("/admin/sandboxes/stats")
async def get_sandbox_stats(request: Request):
    sandbox_manager = request.app.state.sandbox_manager
...
```python
# 强制结束并重建沙箱
await sandbox_manager.end_sandbox(sandbox_id, CleanupReason.ERROR)
result = await sandbox_manager.get_or_create(session_id=session_id)
sandbox = result.sandbox
```
...
- [execution-environment-config.md](./execution-environment-config.md)
- [AI-Agent系统架构设计文档](../../docs/archive/AI-Agent系统架构设计文档.md)（早期愿景，归档）

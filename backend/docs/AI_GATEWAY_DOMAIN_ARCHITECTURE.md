...
│   ├── proxy_use_case.py          # 对外 LLM 代理编排（OpenAI chat；Anthropic 原生 messages）
...
**Proxy 前置校验**：``ProxyUseCase`` 在调用 LiteLLM Router 前会根据 ``GatewayModelRepository.get_by_name`` 校验注册别名 ``capability`` 与当前 HTTP 入口（``chat_completion`` / ``image_generation`` / ``video_generation`` 等）一致；不匹配时抛 ``CapabilityNotAllowedError``（HTTP 400），避免 LiteLLM 返回含糊的 “No deployments available”。

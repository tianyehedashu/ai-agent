-- =============================================================================
-- 生产运维手工执行 | Alembic 运行时不会加载本文件
-- 本地/开发请用: uv run alembic upgrade head  （走 alembic/versions/*.py）
-- versions/20260515_api_key_gateway_grants.py
-- revision: 20260515_akgg
-- down_revision: 20260514_gld
-- 方向: DOWNGRADE (down.sql)
--   up.sql   = 升级（从 down_revision 升到 revision）
--   down.sql = 回滚（从 revision 退回到 down_revision）
-- 执行后请手工维护 alembic_version.version_num
-- =============================================================================

DROP INDEX ix_api_key_gateway_grants_user_team;
DROP INDEX ix_api_key_gateway_grants_is_active;
DROP INDEX ix_api_key_gateway_grants_team_id;
DROP INDEX ix_api_key_gateway_grants_user_id;
DROP INDEX ix_api_key_gateway_grants_api_key_id;
DROP TABLE api_key_gateway_grants;

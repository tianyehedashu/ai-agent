...
- 添加 is_superuser 字段
- 添加 is_verified 字段
- 确保 hashed_password 字段存在（如果存在 password_hash 则重命名）
- 确保所有 fastapi-users 必需字段都存在
"""
...
def upgrade() -> None:
    """升级：添加 fastapi-users 必需的字段"""
    conn = op.get_bind()
    inspector = sa.inspect(conn)
    columns = {col["name"]: col for col in inspector.get_columns("users")}

    # 1. 处理密码字段: 如果存在 password_hash，重命名为 hashed_password
...
def downgrade() -> None:
    ...
    # 3. 将 hashed_password 重命名回 password_hash（如果存在）

"""
Listing Studio Prompt Service - Listing Studio 提示词模板服务
...
from domains.agent.infrastructure.repositories.listing_studio_prompt_template_repository import (
    ListingStudioPromptTemplateRepository,
)
...
def get_capabilities_config() -> dict[str, Any]:
    """能力列表 + 后端计算的 execution_layers（前端编排真源）。"""
    caps = list_capabilities()
    layers = build_execution_layers(list(CAPABILITY_ORDER))
    execution_layers = [[cap_id for _, cap_id in layer] for layer in layers]
    return {"capabilities": caps, "execution_layers": execution_layers}
...
class ListingStudioPromptTemplateUseCase:
    """Listing Studio 提示词模板用例"""

    def __init__(self, db: AsyncSession) -> None:
        self.db = db
        self.repo = ListingStudioPromptTemplateRepository(db)

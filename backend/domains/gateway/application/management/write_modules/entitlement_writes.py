...
class EntitlementWritesMixin:
    ...
    async def upsert_budget(self, *, scope: str, scope_id: uuid.UUID | None, period: str, model_name: str | None=None, limit_usd: Decimal | None, soft_limit_usd: Decimal | None=None, limit_tokens: int | None, limit_requests: int | None) -> Any:
        return await self._budgets.upsert(scope=scope, scope_id=scope_id, period=period, model_name=model_name, limit_usd=limit_usd, soft_limit_usd=soft_limit_usd, limit_tokens=limit_tokens, limit_requests=limit_requests)

    async def delete_budget(self, budget_id: uuid.UUID) -> None:
        await self._budgets.delete(budget_id)

    async def create_alert_rule(self, *, team_id: uuid.UUID, name: str, description: str | None, metric: str, threshold: Decimal, window_minutes: int, channels: dict[str, Any], enabled: bool) -> GatewayAlertRule:
        return await self._alerts.create_rule(team_id=team_id, name=name, description=description, metric=metric, threshold=threshold, window_minutes=window_minutes, channels=channels, enabled=enabled)

    async def update_alert_rule(self, rule_id: uuid.UUID, *, team_id: uuid.UUID, fields: dict[str, Any]) -> GatewayAlertRule:
    ...
    async def update_provider_plan(self, plan_id: uuid.UUID, *, credential_id: uuid.UUID, team_id: uuid.UUID, is_platform_admin: bool, fields: dict[str, Any], quotas: list[dict[str, Any]] | None=None) -> ProviderPlan:
        await self._assert_credential_in_team(credential_id, team_id=team_id, is_platform_admin=is_platform_admin)
    ...
    async def delete_provider_plan(self, plan_id: uuid.UUID, *, credential_id: uuid.UUID, team_id: uuid.UUID, is_platform_admin: bool) -> None:
        await self._assert_credential_in_team(credential_id, team_id=team_id, is_platform_admin=is_platform_admin)
        await self._assert_provider_plan_in_credential(plan_id, credential_id=credential_id)
    ...
    async def create_entitlement_plan(self, *, scope: str, scope_id: uuid.UUID, team_id: uuid.UUID, is_platform_admin: bool, label: str, valid_from: datetime, valid_until: datetime, included_models: list[str] | None=None, included_capabilities: list[str] | None=None, is_active: bool=True, auto_renew: bool=False, notes: str | None=None, extra: dict[str, Any] | None=None, quotas: list[dict[str, Any]] | None=None) -> EntitlementPlan:
        if scope == 'vkey':
            await self._assert_vkey_in_team(scope_id, team_id=team_id, is_platform_admin=is_platform_admin)
        elif scope == 'apikey_grant':
            await self._assert_apikey_grant_in_team(scope_id, team_id=team_id, is_platform_admin=is_platform_admin)
        else:
    ...
    async def update_entitlement_plan(self, plan_id: uuid.UUID, *, team_id: uuid.UUID, is_platform_admin: bool, fields: dict[str, Any], quotas: list[dict[str, Any]] | None=None) -> EntitlementPlan:
        await self._assert_entitlement_plan_in_team(plan_id, team_id=team_id, is_platform_admin=is_platform_admin)
    ...
    async def delete_entitlement_plan(self, plan_id: uuid.UUID, *, team_id: uuid.UUID, is_platform_admin: bool) -> None:
        await self._assert_entitlement_plan_in_team(plan_id, team_id=team_id, is_platform_admin=is_platform_admin)

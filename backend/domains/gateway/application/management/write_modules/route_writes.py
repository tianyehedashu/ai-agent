...
class RouteWritesMixin:
    ...
    async def create_gateway_route(self, *, team_id: uuid.UUID, virtual_model: str, primary_models: list[str], fallbacks_general: list[str], fallbacks_content_policy: list[str], fallbacks_context_window: list[str], strategy: str, retry_policy: dict[str, Any]) -> Any:
        row = await self._routes.create(team_id=team_id, virtual_model=virtual_model, primary_models=primary_models, fallbacks_general=fallbacks_general, fallbacks_content_policy=fallbacks_content_policy, fallbacks_context_window=fallbacks_context_window, strategy=validate_routing_strategy(strategy), retry_policy=retry_policy)
        await self.reload_litellm_router()
        return row

    async def update_gateway_route(self, route_id: uuid.UUID, *, team_id: uuid.UUID, fields: dict[str, Any]) -> Any:
        repo = self._routes

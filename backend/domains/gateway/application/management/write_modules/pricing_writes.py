...
class PricingWritesMixin:
    ...
    async def upsert_upstream_pricing(self, *, provider: str, upstream_model: str, capability: str, currency: str, amount_per_million: dict[str, Any]):
        ...
        existing = await repo.get_active(provider=provider, upstream_model=upstream_model, capability=capability, at=now)
        version = 1
    ...
    async def upsert_downstream_pricing(self, *, scope: str, scope_id: uuid.UUID | None, gateway_model_id: uuid.UUID | None, inheritance_strategy: str, currency: str='CNY', amount_per_million: dict[str, Any] | None=None):
        ...
        if inheritance_strategy == 'mirror':
            row = await repo.create(scope=scope, scope_id=scope_id, gateway_model_id=gateway_model_id, inheritance_strategy='mirror', effective_from=now, version=version)
            from domains.gateway.application.pricing.pricing_resolution_cache import (
                invalidate_pricing_resolution_cache,
            )
            await invalidate_pricing_resolution_cache(
        ...
        row = await repo.create(scope=scope, scope_id=scope_id, gateway_model_id=gateway_model_id, inheritance_strategy='manual', input_cost_per_token=inp, output_cost_per_token=out, cache_creation_input_token_cost=cc, cache_read_input_token_cost=cr, per_request_usd=per_request_usd, extra=extra, effective_from=now, version=version)
        from domains.gateway.application.pricing.pricing_resolution_cache import (
            invalidate_pricing_resolution_cache,
        )
        await invalidate_pricing_resolution_cache(

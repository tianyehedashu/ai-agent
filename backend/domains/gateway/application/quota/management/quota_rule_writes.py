...
from domains.gateway.domain.quota.quota_plan import default_reset_strategy_for_window
...
class QuotaRuleWritesMixin:
    ...
    async def _maybe_index_user_credential_budget(budget: GatewayBudget) -> None:
        if budget.credential_id is None or budget.target_id is None:
            return
        from domains.gateway.application.budget.user_credential_budget_index import (
            add_user_credential,
        )
    ...
    async def _upsert_downstream_quota_rule(
        ...
        *,
        ...
        from domains.gateway.application.quota.management.plan_quota_merge import (

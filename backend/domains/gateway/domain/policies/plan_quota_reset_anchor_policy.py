...
from domains.gateway.domain.quota_plan import normalize_reset_strategy
...
def validate_plan_quota_reset_anchor(
    *,
    ...
    """``window_seconds=0`` / ``rolling`` / ``plan_anniversary`` 禁止非默认锚点。"""
    ...
    if strategy in ("rolling", "plan_anniversary") and not anchor.is_default():
...
def resolve_plan_quota_reset_anchor(
    *,
    ...
    strategy = normalize_reset_strategy(reset_strategy)
    if window_seconds <= 0 or strategy in ("rolling", "plan_anniversary"):

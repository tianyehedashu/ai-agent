...
前缀与 provider 是否匹配的业务校验见 ``domains.gateway.application.upstream.litellm_real_model_prefix``。
"""
...
def strip_litellm_provider_prefix(provider: str, model_id: str) -> str:
    """剥离 LiteLLM provider 前缀，返回裸上游模型 ID（与 ``build_litellm_model_id`` 互逆）。

    落库 ``real_model`` 可能带 provider 前缀（如 ``openai/gpt-4o-mini``），
    而上游定价表 ``upstream_model`` 存裸 ID（如 ``gpt-4o-mini``，见
    ``config_catalog_sync``）。解析定价时需剥前缀以匹配上游定价行。

    仅剥离与 ``provider`` 对应的前缀；其它前缀（如 ``azure/``、``vertex_ai/``）
    原样返回，避免误伤多级路由场景。
    """
    stripped = model_id.strip()
    if "/" not in stripped:
        return stripped
    provider_key = provider.strip().lower()
    expected_prefix = "zai" if provider_key == "zhipuai" else provider_key
    first, _, rest = stripped.partition("/")
    if first == expected_prefix and rest:
        return rest
    return stripped

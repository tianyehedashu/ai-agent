...
def _get_user_ids(current_user: AuthUser) -> tuple[str | None, str | None]:
    """从当前用户获user_id ?anonymous_user_id

    Returns:
        (user_id, anonymous_user_id) 元组，根据用户类型只有一个不None
    """
...
class SessionResponse(BaseModel):
    ...
    id: str
    user_id: str | None  # 注册用户 ID（匿名用户为 None?    anonymous_user_id: str | None  # 匿名用户 ID（注册用户为 None?    agent_id: str | None
    title: str | None
    status: str
    message_count: int
    token_count: int
...
async def generate_session_title(
    ...
    strategy: Annotated[str, Query(description="生成策略: first_message ?summary")] = "summary",
) -> SessionResponse:
    """生成会话标题"""
    session = await session_service.get_session_or_raise(session_id)
    check_session_ownership(session, current_user)

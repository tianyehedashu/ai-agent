...
class TestListingStudioJobApi:
    ...
    async def test_get_job_succeeds(
        ...
        r = await dev_client.get(
            f"/api/v1/listing-studio/jobs/{job_id}",
            headers=auth_headers,
        )
        assert r.status_code == status.HTTP_200_OK
        data = r.json()
    ...
    async def test_get_job_not_found(
        ...
        """不存在的任务返回 404"""
        r = await dev_client.get(
            "/api/v1/listing-studio/jobs/00000000-0000-0000-0000-000000000000",
            headers=auth_headers,
        )
...
@pytest.mark.integration
class TestListingStudioCapabilitiesApi:
    """能力与默认提示词 API"""

    @pytest.mark.asyncio
    async def test_list_capabilities_succeeds(self, dev_client: AsyncClient):
        """能力列表（无需认证）"""
        r = await dev_client.get("/api/v1/listing-studio/capabilities")
        assert r.status_code == status.HTTP_200_OK
        data = r.json()
        ...
        assert len(caps) >= 5
        assert isinstance(data["execution_layers"], list)
        ids = [x["id"] for x in caps]
    ...
    async def test_get_default_prompt_succeeds(self, dev_client: AsyncClient):
        """默认提示词"""
        r = await dev_client.get(
            "/api/v1/listing-studio/capabilities/product_link_analysis/default-prompt"
        )
        assert r.status_code == status.HTTP_200_OK
        data = r.json()
...
@pytest.mark.integration
class TestListingStudioRunStepApi:
    """执行单步 API（mock LLM）"""
    ...
    async def test_run_step_succeeds(
        ...
        with patch.dict(
            ...
            r = await dev_client.post(
                f"/api/v1/listing-studio/jobs/{job_id}/steps",
                json={
...
@pytest.mark.integration
class TestListingStudioStepAndOptimizeApi:
    ...
    async def test_optimize_prompt_endpoint(
        ...
        with patch(
            "domains.agent.application.listing_studio_use_case.optimize_prompt_for_capability",
            side_effect=_mock_optimize,
        ):
            r = await dev_client.post(
                f"/api/v1/listing-studio/jobs/{job_id}/optimize-prompt",
                json={
                    "capability_id": "product_link_analysis",
                    "user_input": {"product_link": "https://a.com"},
...
@pytest.mark.integration
class TestListingStudioRunPipelineApi:
    """一键异步执行 API"""
    ...
    async def test_run_pipeline_returns_202(
        ...
        with patch(
            "domains.agent.presentation.listing_studio_router.run_pipeline_async",
            _noop_pipeline,
        ):
            r = await dev_client.post(
                "/api/v1/listing-studio/run",
...
@pytest.mark.integration
class TestListingStudioTemplatesApi:
    """用户模板 API"""
    ...
    async def test_list_templates_succeeds(
        ...
        """模板列表"""
        r = await dev_client.get(
            "/api/v1/listing-studio/capabilities/product_link_analysis/templates",
            headers=auth_headers,
        )
        assert r.status_code == status.HTTP_200_OK
        data = r.json()
    ...
    async def test_create_and_get_template_succeeds(
        ...
        get_r = await dev_client.get(
            f"/api/v1/listing-studio/templates/{template_id}",
            headers=auth_headers,
        )
...
@pytest.mark.integration
class TestListingStudioLegacyProductInfoApi:
    """旧 /product-info 路径仍可用，并返回 Deprecation 头"""

    @pytest.mark.asyncio
    async def test_legacy_path_returns_deprecation_headers(
        self,
        dev_client: AsyncClient,
    ):

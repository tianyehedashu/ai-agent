...
@pytest_asyncio.fixture
async def permission_context(test_user: User, db_session: AsyncSession):
    """权限上下文 fixture - 为测试用户设置权限上下文（含 team_ids）。"""
    from tests.helpers.permission_context import permission_context_for_user

    ctx = await permission_context_for_user(db_session, user_id=test_user.id)

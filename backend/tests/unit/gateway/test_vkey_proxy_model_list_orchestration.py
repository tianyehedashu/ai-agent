...
async def test_multi_grant_homonym_slug_skips_grant_team_models(db_session: object) -> None:
    ...
    with (
        patch(
            "domains.gateway.application.vkey_proxy_model_list.GatewayManagementReadService",
            return_value=reads,
        ),
        patch(
            "domains.gateway.application.vkey_proxy_model_list.fetch_grant_team_slug_rows",
            AsyncMock(
                return_value=[
                    (bound, "primary-slug"),
                    (grant_a, dup_slug),
        ...
        patch(
            "domains.gateway.application.vkey_proxy_model_list.build_proxy_models_list",

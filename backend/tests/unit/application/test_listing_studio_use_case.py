...
@pytest.mark.unit
class TestListingStudioConstants:
    """产品信息常量"""
...
@pytest.mark.unit
class TestListingStudioUseCase:
    """ListingStudioUseCase 单元测试"""
    ...
    async def test_run_step_image_analysis_requires_vision_model_raises(self, db_session):
        ...
        try:
            ...
            with (
                patch.object(
                    use_case._model_resolution,
                    "visible_image_system_model_ids",
                    new_callable=AsyncMock,
                    return_value=frozenset(["qwen-vl-max"]),
                ),
                patch.object(
                    use_case._model_resolution,
                ...
                mock_resolve_vision.return_value = ResolvedModel(model="deepseek-chat")

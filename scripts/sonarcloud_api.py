...
PROJECTS = [
    {"key_suffix": "ai-agent-backend", "name": "Backend (Python)", "language": "python"},
    {"key_suffix": "ai-agent-frontend", "name": "Frontend (TypeScript)", "language": "typescript"},
]
...
def cmd_issues(client: SonarCloudClient, args: argparse.Namespace) -> None:
    ...
    for proj in PROJECTS:
        ...
        all_issues.extend([
            {**issue.__dict__, "project": proj["name"]}
            for issue in issues
        ])
...
示例:
  # 使用 .env 文件配置 (推荐)
  python sonarcloud_api.py report --format html
  python sonarcloud_api.py metrics

  # 或通过命令行指定
  python sonarcloud_api.py --org myorg report --format html
...
环境变量:
  ...
  SONAR_ORGANIZATION  组织名 (可选，可用 --org 覆盖)
        """,
    )
    ...
    if not token:
        ...
        print("")
        print("请通过以下方式之一配置:")
        print("  1. 在 .env 文件中添加: SONAR_TOKEN=your-token")
        print("  2. 设置环境变量: export SONAR_TOKEN=your-token")
        sys.exit(1)
    ...
    if not args.org:
        ...
        print("")
        print("请通过以下方式之一配置:")
        print("  1. 在 .env 文件中添加: SONAR_ORGANIZATION=your-org")
        print("  2. 使用命令行参数: --org your-org")
        sys.exit(1)

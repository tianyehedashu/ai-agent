...
## 更新摘要
**所做更改**
- 更新了配额规则缓存架构，移除了进程内LRU快照缓存，简化为纯Redis缓存架构
- 更新了缓存层次结构图，反映从双层缓存到单层Redis缓存的转变
- 更新了配额规则缓存的实现细节，强调Redis作为唯一真实来源的重要性
- 更新了缓存失效机制，从版本号失效改为直接Redis键失效
...
**重要更新**：缓存架构已显著简化，移除了进程内LRU快照缓存，服务现在完全依赖Redis作为配额快照的唯一真实来源，消除了复杂的双层缓存系统。
...
A5["quota_rule_cache.py<br/>配额规则缓存<br/>(纯Redis架构)"]
end
subgraph "配置层"
...
- [quota_rule_cache.py:1-388](file://backend/domains/gateway/application/management/quota_rule_cache.py#L1-L388)
- [budget_config_cache.py](file://backend/domains/gateway/application/budget_config_cache.py)
- [system_grants_cache.py](file://backend/domains/gateway/application/system_grants_cache.py)
- [prompt_cache.py](file://backend/domains/agent/infrastructure/llm/prompt_cache.py)
- [checkpoint_cache.py](file://backend/domains/agent/infrastructure/memory/checkpoint_cache.py)
...
#### 4.5 配额规则缓存（重大更新）
- **架构更新**：完全移除进程内LRU快照缓存，采用纯Redis缓存架构
- 目标：提供配额规则的高效缓存，支持版本号失效机制
- 实现要点：
  - 使用Redis存储配额规则快照，键格式为`gw:quota_rules:{version}:{cache_key}`
  - 通过版本号键`gw:quota_rules:ver:{team_id}`实现缓存失效
  - 自动序列化/反序列化QuotaRuleReadModel对象
- 失效策略：通过递增版本号实现缓存失效，无需本地缓存同步
...
RedisLib --> QuotaRules["quota_rule_cache.py<br/>(纯Redis架构)"]
...
**重大架构更新**：配额规则缓存已简化为纯Redis架构，移除了进程内LRU快照缓存，通过版本号机制实现缓存失效，降低了系统复杂度并提高了缓存一致性。
...
- 建议的键前缀
  ...
  - 配额规则：gw:quota_rules:{version}:{cache_key}
  - 配额规则版本：gw:quota_rules:ver:{team_id}
...
## 更新摘要
**所做更改**
- 新增Docker清理操作超时保护机制章节
- 更新PersistentDockerExecutor超时配置说明
- 增强故障排除指南中的Docker守护进程问题处理
- 完善监控与审计章节中的超时检测机制
...
**更新** 新增Docker清理操作的超时保护机制，确保在Docker守护进程不可用时防止系统无限挂起。
...
subgraph "执行器层"
PDE["PersistentDockerExecutor<br/>沙箱级容器<br/>超时保护机制"]
...
**更新** PersistentDockerExecutor新增超时保护机制，包括10秒超时检测docker ps和30秒超时执行docker rm，防止Docker守护进程不可用时的无限挂起。
...
Exec-->>API : 返回执行结果
API-->>Client : 响应结果
Note over SM,Docker : 空闲超时/配额不足触发清理与淘汰<br/>Docker守护进程异常时超时保护机制生效
```
...
+cleanup_orphaned_containers(max_age)
}
...
### 执行器层（Executor）
- PersistentDockerExecutor：为每个沙箱维持一个长期运行的容器，适合需要保持状态的场景。**新增超时保护机制**：使用10秒超时检测docker ps和30秒超时执行docker rm，防止Docker守护进程不可用时的无限挂起。
...
### Docker清理操作超时保护机制
**新增** PersistentDockerExecutor实现了双重超时保护机制，确保在Docker守护进程异常时系统的稳定性：

- **docker ps探测超时**：10秒超时保护，防止长时间等待Docker守护进程响应
- **docker rm清理超时**：30秒超时保护，确保清理操作不会无限期挂起
- **异常处理**：当Docker命令超时或OSError时，记录警告信息并优雅降级

```mermaid
flowchart TD
Start(["开始清理"]) --> Probe["docker ps 探测<br/>超时: 10秒"]
Probe --> Timeout{"超时/异常?"}
Timeout --> |是| Skip["跳过清理<br/>记录警告"]
Timeout --> |否| CheckContainers["检查容器列表"]
CheckContainers --> HasContainers{"有容器需要清理?"}
HasContainers --> |否| End["结束"]
HasContainers --> |是| Remove["docker rm -f<br/>超时: 30秒"]
Remove --> Success{"删除成功?"}
Success --> |是| Log["记录清理日志"]
Success --> |否| Error["记录错误日志"]
Log --> End
Error --> End
Skip --> End
```
...
**图表来源**
- [executor.py: 435-456:435-456](file://backend/domains/agent/infrastructure/sandbox/executor.py#L435-L456)

**章节来源**
- [executor.py: 256-259:256-259](file://backend/domains/agent/infrastructure/sandbox/executor.py#L256-L259)
- [executor.py: 435-456:435-456](file://backend/domains/agent/infrastructure/sandbox/executor.py#L435-L456)
...
- **超时保护**：新增的超时机制确保在Docker守护进程异常时系统能够快速失败并恢复，避免资源浪费。
...
- **Docker守护进程异常**
  - 现象：清理操作卡住或超时，系统响应缓慢。
  - 处理：检查Docker守护进程状态，确认网络连接正常；系统会自动应用10秒超时保护机制。
  - 参考：[executor.py: 435-456:435-456](file://backend/domains/agent/infrastructure/sandbox/executor.py#L435-L456)
- **清理僵尸容器**
  - 操作：执行清理脚本，移除长时间未使用的沙箱容器。
  - 参考：[cleanup_sandbox_containers.py](file://backend/scripts/cleanup_sandbox_containers.py)

**更新** 新增Docker守护进程异常的故障排除指导，强调超时保护机制的作用。
...
## 结论
该沙箱执行环境通过"配置驱动+多执行器+强隔离"的设计，在保证安全性的同时兼顾性能与可运维性。Docker持久化容器提供稳定的执行上下文，配合严格的资源与安全策略，能够有效防止越权与资源滥用。完善的监控与审计能力有助于持续改进与合规要求。

**更新** 新增的超时保护机制进一步增强了系统的鲁棒性，确保在Docker守护进程异常时能够快速失败并恢复，避免无限挂起导致的系统资源浪费。建议在生产中结合环境配置文件与配额策略，持续优化镜像与网络策略，确保系统稳定高效运行。
...
### 超时保护配置详解
**新增** PersistentDockerExecutor的超时保护配置：

- **DOCKER_PROBE_TIMEOUT_SECONDS**: 10秒
  - 用途：检测Docker守护进程状态和列出容器的超时时间
  - 影响：防止docker ps命令在Docker守护进程异常时无限挂起
  - 场景：容器状态检查、清理操作前的状态确认

- **DOCKER_RM_TIMEOUT_SECONDS**: 30秒  
  - 用途：执行容器删除操作的超时时间
  - 影响：确保docker rm命令不会长时间阻塞系统
  - 场景：批量清理容器、系统关闭时的资源回收

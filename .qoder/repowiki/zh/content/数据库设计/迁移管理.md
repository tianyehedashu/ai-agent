...
- [backend/alembic/sql/20260616_downstream_pricing_partial_index.up.sql](file://backend/alembic/sql/20260616_downstream_pricing_partial_index.up.sql)
- [backend/alembic/sql/20260616_downstream_pricing_partial_index.down.sql](file://backend/alembic/sql/20260616_downstream_pricing_partial_index.down.sql)
</cite>
...
**更新** 本次更新反映了新增的数据库迁移脚本，包括部分索引优化和凭证创建者修复功能，以及相应的迁移管理策略更新。特别关注了保守回滚设计原则和手工运维脚本的使用。
...
I["fix_credential_created_by_user_id.py<br/>手工修复脚本"] --> J["provider_credentials表修复"]
K["app*.toml / environments/*<br/>多环境配置"] --> B
L["libs/db/__init__.py<br/>ORM元数据"] --> B
M["新增迁移脚本<br/>部分索引/凭证修复"] --> C
```
...
- 工具脚本
  ...
  - fix_credential_created_by_user_id.py：专门的手工修复脚本，用于处理历史数据问题。
- 多环境配置
...
#### 部分索引优化迁移
新增的部分索引优化脚本提升了数据库查询性能：

- **迁移脚本**：20260616_downstream_pricing_partial_index.py
- **功能**：为downstream_model_pricing表创建部分索引ix_downstream_model_pricing_lookup_active
- **实现**：仅覆盖effective_to IS NULL的活跃行，使用IMMUTABLE条件满足PostgreSQL部分索引要求
- **性能提升**：减少热路径查询的索引扫描量，提升pricing解析性能
- **保守回滚**：回滚时删除索引，不影响数据完整性
...
- **SQL脚本**：20260616_fix_credential_created_by_user_id.up.sql
- **功能**：修复team_members表中created_by_user_id为NULL的历史数据
- **实现**：根据最早的团队成员关系回填创建者信息，选择最早加入的管理员或成员
- **保守设计**：down.sql不执行回滚操作，避免数据丢失
- **手工执行**：生产环境使用，Alembic运行时不加载本文件
...
- **工具脚本**：fix_credential_created_by_user_id.py
- **功能**：异步修复provider_credentials表中created_by_user_id为NULL的记录
- **实现**：查询所有需要修复的凭据，逐条查找最早加入的团队成员并更新
- **安全措施**：使用事务控制，打印修复进度，避免数据丢失
- **使用方法**：uv run python -m scripts.fix_credential_created_by_user_id
...
- 历史数据修复：fix_credential_created_by_user_id.py专门处理历史数据问题，提供安全的修复机制。
...
- 保守回滚设计：对于历史数据修复等高风险操作，采用保守回滚策略（如凭证修复脚本的down.sql不执行回滚）。
...
ProdMig --> ManualFix["手工修复<br/>fix_credential_created_by_user_id.py"]
ManualFix --> End(["结束"])
```
...
- 工具依赖
  ...
  - fix_credential_created_by_user_id.py依赖数据库会话和模型定义。
...
ManualFix["fix_credential_created_by_user_id.py"] --> DB["数据库会话"]
ManualFix --> Models["模型定义"]
```
...
- 常见问题
  ...
  - 历史数据修复失败：检查团队成员关系是否完整，确认数据一致性。
- 回滚与恢复
  - 使用downgrade回退至上一版本，验证业务可用性。
  - 若回滚失败，结合运维SQL文件进行手工修复。
  - 对于保守回滚的脚本（如凭证修复），使用手工SQL进行修复。
- 日志与审计
...
## 结论
本项目已建立完善的Alembic迁移体系：从配置、脚本、工具到多环境与测试，形成闭环。新增的迁移脚本进一步增强了系统的功能性和可维护性，包括部分索引优化、历史数据修复和保守回滚设计等重要特性。特别是凭证创建者修复脚本展现了高级的数据迁移技术和保守的回滚设计原则，为后续的数据库演进提供了坚实的基础。遵循本文档的命名规范、版本控制策略、安全措施与性能优化建议，可有效保障数据库演进的稳定性与可维护性。

**更新** 最新的迁移脚本展示了高级的数据迁移技术，包括复杂的数据转换、批量处理、部分索引优化和保守的回滚设计原则，为后续的数据库演进提供了坚实的基础。
...
#### 保守回滚设计原则
新增的凭证修复脚本体现了保守回滚设计原则：

- **回滚保护**：down.sql不执行任何操作，避免数据丢失
- **手工修复**：提供up.sql和手工修复脚本，确保数据完整性
- **安全验证**：在修复前进行数据验证，确认修复逻辑正确
- **渐进式修复**：分步执行修复，避免一次性大规模更改
...
#### 性能优化技术
新增的部分索引优化脚本体现了以下性能优化技术：

- **部分索引**：仅覆盖活跃数据，减少索引维护成本
- **IMMUTABLE条件**：满足PostgreSQL部分索引谓词要求
- **查询优化**：针对热路径查询创建专用索引
- **数据分区**：通过索引策略实现逻辑分区效果

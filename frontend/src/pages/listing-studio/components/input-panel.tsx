...
import { type ListingStudioInputs, FIELD_ICONS, getTextInputValue } from './input-panel-shared'
...
interface InputPanelProps {
  ...
  onChange: (inputs: ListingStudioInputs) => void
  disabled?: boolean
  /** 侧栏紧凑模式：单列、更小间距 */
  compact?: boolean
}
            ...
            <div className="space-y-3">
              {productFields.map(({ key, label, placeholder }) => (
                <FieldRow
                  ...
                  label={label}
                  placeholder={placeholder}
                  value={getTextInputValue(inputs, key)}
                  onChange={(v) => {

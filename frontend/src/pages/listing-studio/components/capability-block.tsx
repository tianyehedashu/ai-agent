...
import type {
  ...
  ListingStudioStepStatus,
} from '@/types/listing-studio'

import { inputsToUserInput, type ListingStudioInputs } from './input-panel-shared'
...
export function CapabilityBlock({
  ...
  const { data: defaultPromptRes } = useQuery({
    queryKey: ['listing-studio', 'default-prompt', capabilityId],
    queryFn: () => listingStudioApi.getDefaultPrompt(capabilityId),
    enabled: expanded,
  })
  const defaultPrompt = defaultPromptRes?.content ?? ''

  const { data: templatesData } = useQuery({
    queryKey: ['listing-studio', 'templates', capabilityId],
    queryFn: () => listingStudioApi.listTemplates(capabilityId, { limit: 50 }),
    enabled: expanded,
  })
  ...
  const updateJobCache = useCallback(
    (data: ListingStudioJob) => {
      queryClient.setQueryData(['listing-studio', 'job', data.id], data)
  ...
  const optimizePromptMutation = useMutation({
    mutationFn: async () => {
      ...
      return listingStudioApi.optimizePrompt(jobId, {
  ...
  const executeMutation = useMutation({
    mutationFn: async () => {
      const jobId = job?.id ?? (ensureJob ? await ensureJob() : null)
      if (!jobId) throw new Error('请先创建任务')
      return listingStudioApi.runStep(jobId, {
        capability_id: capabilityId,
        user_input: buildUserInput(),
        model_id: selectedModelId,
        meta_prompt: metaPrompt === '' ? undefined : metaPrompt,
    ...
    onError: (err) => {
      ...
      const needsGateway = /无可用|Gateway|凭据|同步目录/.test(description)
      toast({
        title: '执行失败',
        description: needsGateway
          ? `${description}。请前往「AI 网关 → 凭据/模型」完成配置。`
          : description,
        variant: 'destructive',
      })
    },
  ...
  const actualStatus: ListingStudioStepStatus = optimizePromptMutation.isPending
    ? 'prompt_generating'
  ...
  const saveTemplateMutation = useMutation({
    mutationFn: async (templateName: string) => {
      ...
      return listingStudioApi.createTemplate(capabilityId, {
        name: templateName,
  ...
  const handleLoadTemplate = (t: ListingStudioPromptTemplate): void => {
